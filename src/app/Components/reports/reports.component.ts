...
export class ReportsComponent implements OnInit {
	...
	loadSavedReports(): void {
		this.reportStorageService.getAllReports().subscribe({
			next: (reports) => {
	...
	saveCurrentReport(): void {
		...
		this.reportStorageService.saveReport(reportFile).subscribe({
			next: () => {
				...
				this.loadSavedReports(); // Refresh list after saving
			},
			error: (error) => {
    ...
    try {
      ...
      const columnParts = this.selectedAttributes.map(attr => `${attr.table}.${attr.column}`);
      selectPart += columnParts.join(', ');
       
      // Generate FROM part with JOINs if needed
      const primaryTable = this.selectedTables[0];
      let fromPart = ` FROM ${primaryTable}`;
      
      // Build JOIN statements directly based on table and column names
      if (this.selectedTables.length > 1) {
        // Create a map of tables to their selected columns
        const tableColumns = new Map<string, string[]>();
        
        this.selectedAttributes.forEach(attr => {
          if (!tableColumns.has(attr.table)) {
            tableColumns.set(attr.table, []);
          }
          tableColumns.get(attr.table)?.push(attr.column);
        });
        
        // Identify common column names between tables (potential join keys)
        for (let i = 1; i < this.selectedTables.length; i++) {
          const secondaryTable = this.selectedTables[i];
          let joinCondition = '';
          
          // Check for common ID columns based on exact name match
          for (const table1 of this.selectedTables.slice(0, i)) {
            const table1Cols = tableColumns.get(table1) || [];
            const table2Cols = tableColumns.get(secondaryTable) || [];
            
            // Collect all potential common columns for inspection
            const commonCols = [];
            
            // First, check for columns ending with "id" or "Id"
            for (const col1 of table1Cols) {
              if (col1.toLowerCase().endsWith('id')) {
                // Check if same column exists in second table
                for (const col2 of table2Cols) {
                  if (col1.toLowerCase() === col2.toLowerCase()) {
                    commonCols.push({ col1, col2 });
                  }
                }
              }
            }
            ...
            if (commonCols.length > 0) {
              // Use the first common column as the join condition
              const { col1, col2 } = commonCols[0];
              joinCondition = ` INNER JOIN ${secondaryTable} ON ${table1}.${col1} = ${secondaryTable}.${col2}`;
              break;
            }
            
            // If no common column found, look for foreign key patterns
            if (joinCondition === '') {
              // Look for tablename + id pattern (e.g., patientid in patients and billing)
              const table1Name = table1.toLowerCase().replace(/s$/, ''); // Remove trailing 's'
              const table2Name = secondaryTable.toLowerCase().replace(/s$/, ''); // Remove trailing 's'
              
              // Check if table1id exists in table2
              const table1IdCol = `${table1Name}id`;
              if (table2Cols.some(col => col.toLowerCase() === table1IdCol)) {
                const table1IdColExact = table2Cols.find(col => col.toLowerCase() === table1IdCol);
                joinCondition = ` INNER JOIN ${secondaryTable} ON ${table1}.${table1IdCol} = ${secondaryTable}.${table1IdColExact}`;
                break;
              }
              
              // Check if table2id exists in table1
              const table2IdCol = `${table2Name}id`;
              if (table1Cols.some(col => col.toLowerCase() === table2IdCol)) {
                const table2IdColExact = table1Cols.find(col => col.toLowerCase() === table2IdCol);
                joinCondition = ` INNER JOIN ${secondaryTable} ON ${table1}.${table2IdColExact} = ${secondaryTable}.${table2IdCol}`;
                break;
              }
            }
          }
          
          // If no join condition found, use a hardcoded mapping for common database patterns
          if (joinCondition === '') {
            // Hardcoded mappings for common table relationships
            const knownRelationships: {[key: string]: {sourceTable: string, targetTable: string, sourceCol: string, targetCol: string}} = {
              'patients_billing': {
              ...
              // Add more known relationships here
            };
            
            // Check for relationships in both directions
            for (const [key, relation] of Object.entries(knownRelationships)) {
              if ((relation.sourceTable === primaryTable && relation.targetTable === secondaryTable) ||
                  (relation.sourceTable === secondaryTable && relation.targetTable === primaryTable)) {
                
                if (relation.sourceTable === primaryTable) {
                  joinCondition = ` INNER JOIN ${secondaryTable} ON ${primaryTable}.${relation.sourceCol} = ${secondaryTable}.${relation.targetCol}`;
                } else {
                  joinCondition = ` INNER JOIN ${secondaryTable} ON ${secondaryTable}.${relation.sourceCol} = ${primaryTable}.${relation.targetCol}`;
                }
                break;
              }
            }
          ...
          // If still no join condition, default to CROSS JOIN with a warning
          if (joinCondition === '') {
            console.warn(`No join condition found for ${secondaryTable}. Using CROSS JOIN.`);
            joinCondition = ` CROSS JOIN ${secondaryTable}`;
          }
          
          fromPart += joinCondition;
        }
      }
      ...
      if (this.activeFilters.length > 0) {
        wherePart = ' WHERE ' + this.activeFilters.map(filter => {
          if (!filter.field || !filter.value) return null;
          const field = filter.field.includes('.') ? filter.field : `${this.currentDisplayTable}.${filter.field}`;
          return filter.operator === 'equals' ? `${field} = '${filter.value}'`
               : filter.operator === 'contains' ? `${field} LIKE '%${filter.value}%'`
               : filter.operator === 'greaterThan' ? `${field} > ${filter.value}`
               : filter.operator === 'lessThan' ? `${field} < ${filter.value}`
               : null;
        }).filter(Boolean).join(' AND ');
      }
      
      this.sqlQuery = selectPart + fromPart + wherePart; // Store generated SQL query
      console.log("Generated SQL Query:", this.sqlQuery);
    } catch (error) {
	...
	parseSqlQuery() {
		...
		try {
			...
			const fromMatch = sql.match(
				/FROM\s+([^WHERE|GROUP BY|ORDER BY|HAVING|LIMIT]+)/i
			);
			if (fromMatch && fromMatch[1]) {
				...
				if (
					...
					(!this.currentDisplayTable ||
						!this.selectedTables.includes(this.currentDisplayTable))
				) {
					this.setCurrentDisplayTable(this.selectedTables[0]);
				}
			}
			...
			const whereMatch = sql.match(
				/WHERE\s+(.+?)(?:\s+GROUP BY|\s+ORDER BY|\s+HAVING|\s+LIMIT|$)/i
			);
			...
			const groupByMatch = sql.match(
				/GROUP BY\s+(.+?)(?:\s+ORDER BY|\s+HAVING|\s+LIMIT|$)/i
			);
    ...
    const encodedQuery = encodeURIComponent(this.sqlQuery);
    
    // Use the correct API endpoint format
    this.http.get<any[]>(`${this.baseUrl}/table/${encodedQuery}`).subscribe(
      (data) => {
        ...
        try {
          ...
          // Create attribute objects that match the format expected by processReportData
          this.selectedAttributes = columnNames.map(column => ({
            table: '',  // Default to empty since SQL query might not specify table
            column: column,
            displayName: column
          }));
          
          console.log('Created selected attributes:', this.selectedAttributes);
          
          // For SQL queries, we should format the data as non-joined
          // Since we've mapped the columns to match the expected format
          this.processReportData(data, false);
          
          console.log('Processed report data result:', this.reportData);
        } catch (e) {
		...
		this.http.get<any[]>(`${this.baseUrl}/schema/${tableName}`).subscribe(
			(schema) => {
				...
				const normalizedSchema = schema.map((column) => {
					...
					let friendlyType = this.getSqlFriendlyType(
	...
	selectAttribute(attribute: any) {
		const exists = this.selectedAttributes.some(
			(attr) =>
				attr.table === attribute.table && attr.column === attribute.column
		);
	...
	selectAllAttributes() {
		const newAttributes = this.availableAttributes.filter((attr) => {
			return !this.selectedAttributes.some(
				(selected) =>
					selected.table === attr.table && selected.column === attr.column
			);
		});
	...
	generateReport() {
		...
		if (this.selectedTables.length > 0) {
			...
			if (this.selectedTables.length > 1) {
				...
				this.http
					.get<any[]>(`${this.baseUrl}/data/${primaryTable}`, {
						params: queryParams,
					})
					.subscribe(
						(data) => {
						...
						(error) => {
							console.error(
								`Error generating report for ${primaryTable}`,
								error
							);
							this.isLoading = false;
						}
		...
		if (this.selectedGroupBy && this.selectedGroupBy !== "") {
			...
			data.forEach((item) => {
				...
				const groupValue = isJoinedData
					? item[tableName]?.[columnName] !== null &&
					  item[tableName]?.[columnName] !== undefined
						? String(item[tableName][columnName])
						: "null"
				...
				this.selectedAttributes.forEach((attr) => {
					...
					if (isJoinedData) {
						...
						result[attr.displayName] =
							item[attrTable]?.[attrColumn] !== undefined
								? item[attrTable][attrColumn]
								: "N/A";
					} else {
						// For single table data, direct property access
						result[attr.displayName] =
							item[attrColumn] !== undefined ? item[attrColumn] : "N/A";
					}
				});
		...
		if (this.selectedGroupBy !== "") {
			return data.map((item) => {
				...
				this.selectedAttributes.forEach((attr) => {
					...
					result[key] =
						item[attr.column] !== undefined
							? item[attr.column]
							: item[attr.displayName] !== undefined
							? item[attr.displayName]
							: null;
				});
			...
			return data.map((item) => {
				...
				this.selectedAttributes.forEach((attr) => {
					...
					result[qualifiedKey] =
						item[simpleKey] !== undefined
							? item[simpleKey]
							: item[qualifiedKey] !== undefined
							? item[qualifiedKey]
							: item[attr.table]?.[simpleKey] !== undefined
							? item[attr.table][simpleKey]
							: null;
				});
