	...
	generateSqlFromSelections() {
		...
		try {
			...
			if (this.activeFilters.length > 0) {
				wherePart = " WHERE ";
				const filterParts = this.activeFilters
					.map((filter) => {
						...
						const field =
							fieldParts.length === 2
								? filter.field
								: `${this.currentDisplayTable}.${filter.field}`;
		...
		setTimeout(() => {
			...
			if (columnsMatch && columnsMatch[1]) {
				...
				if (columnsMatch[1].trim() === "*") {
					...
					if (
						this.currentDisplayTable &&
						this.tableSchemas[this.currentDisplayTable]
					) {
						columns = this.tableSchemas[this.currentDisplayTable].map(
							(col) => col.name
						);
					}
				} else {
